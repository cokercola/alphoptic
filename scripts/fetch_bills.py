...
CLASSIFICATION_SCHEMA_VERSION = 4  # v4: impact_score replaced with a 7-factor weighted rubric (see CLASSIFY_PROMPT); impact_score/market_relevance now computed deterministically by this script, never asked of Claude
...
def fetch_bill_cbo_estimate(congress, bill_type, number):
    """Returns a prompt-ready context sentence if this bill has a CBO cost
    estimate, otherwise "". Most bills never get one - cboCostEstimates is
    only populated for bills that have advanced far enough - so this is
    a small bonus for the Federal Fiscal Impact factor, not something
    every bill will have. Mirrors fetch_bill_summary's shape: a container
    field on the base bill response pointing at a sub-resource, same
    pattern as cosponsors."""
    url = f"{CONGRESS_BASE}/bill/{congress}/{bill_type}/{number}"
    params = {"api_key": CONGRESS_API_KEY, "format": "json"}
    resp = requests.get(url, params=params, timeout=30)
    resp.raise_for_status()
    cbo = resp.json().get("bill", {}).get("cboCostEstimates", [])
    if not cbo:
        return ""
    description = cbo[0].get("description", "")
    if not description:
        return ""
    return f"CBO estimate: {description} Weigh this directly in your fiscal score."
...
{{
  ...
  "impact_rationale": "one sentence explaining what's actually driving the
    ...
    If the summary field below is empty/unavailable, say so explicitly
    here and explain the score is speculative as a result.",
  "companies": [
...
factors even if its title sounds significant. If there is no summary
text available, do not guess a moderate score from the title alone -
score every factor near the floor and say why in impact_rationale.
...
# Any factor total is capped by these maxes - used to validate Claude's
# response and to compute market_relevance bands consistently with the
# dashboard's "High impact bills" stat and the Bills page filter.
IMPACT_FACTOR_MAX = {
    "economic": 30, "reach": 20, "fiscal": 15, "regulatory": 15,
    "industry_exposure": 10, "state_local": 5, "duration": 5,
}
...
FALLBACK_CLASSIFICATION = {
    ...
    "impact_breakdown": {k: 0 for k in IMPACT_FACTOR_MAX},
    "impact_rationale": "Classification failed for this bill this run - will retry next run.",
    "companies": [],
...
def classify(title, status, summary, cbo_context="", bill_id="unknown"):
    message = client.messages.create(
        ...
        max_tokens=700,  # was 600 - impact_breakdown + impact_rationale + secondary_industries need more room than a single impact_score integer did
        messages=[{
            "role": "user",
...
def compute_impact_score_and_relevance(impact_breakdown, has_summary):
    """The total and the relevance label are ALWAYS computed here, never
    trusted from Claude's own arithmetic - this guarantees the number
    shown on the dashboard, the Bills page filter, and a bill's own
    scorecard can never drift apart from each other.

    market_relevance is a data-confidence axis, not just a magnitude
    one: WATCH overrides whatever the score says, because a score
    computed with no summary text is close to a guess. HIGH is reserved
    for bills that are BOTH scored 70+ AND backed by a real summary -
    that's what makes "High Impact Bills" mean something a person can
    trust rather than just "the biggest number we happened to produce
    from a title alone." See the 2026-08-20 impact rubric discussion
    for why this split exists.
    """
    # Clamp each factor to its allowed max in case Claude returns
    # something out of range - keeps one bad response from silently
    # inflating the total past 100.
    score = sum(
        max(0, min(int(impact_breakdown.get(factor, 0)), max_val))
        for factor, max_val in IMPACT_FACTOR_MAX.items()
    )
    if not has_summary:
        relevance = "WATCH"
    elif score >= 70:
    ...
    index_rows = [
        {
            ...
            # The Bills page's High Impact / Watch List tabs filter on
            # this, not on impact_score directly - see
            # compute_impact_score_and_relevance() for why a raw score
            # threshold alone isn't enough to call something "high impact."
            "market_relevance": s.get("market_relevance"),
...
def main():
    ...
    for bill_id, ref in all_refs.items():
        ...
        # Cache is reused only if: status is unchanged, schema_version
        # matches, every field we're about to read is actually present
        # (guards against malformed records from e.g. a backfill run
        # that stamped a valid schema_version onto an incomplete
        # classification), AND the cached record isn't a WATCH bill.
        # That last condition is deliberate: a WATCH bill has no
        # Congress.gov summary yet, and its status text can easily stay
        # unchanged ("Referred to the Committee on X") for months even
        # after a real summary finally gets published. Without this,
        # WATCH bills would never get reclassified once cached, no
        # matter how much new information becomes available - so they
        # always get a fresh summary fetch + reclassification, on every
        # run, until they graduate out of WATCH.
        REQUIRED_CACHE_FIELDS = (
            "industry", "secondary_industries", "direction", "confidence",
            "summary", "impact_breakdown", "impact_rationale", "companies",
            "has_summary", "market_relevance",
        )
        if (cached and cached.get("status") == status
            ...
            try:
                cbo_context = fetch_bill_cbo_estimate(ref["congress"], ref["type"], ref["number"])
            except requests.exceptions.RequestException as e:
                print(f"WARNING: CBO estimate fetch failed for {bill_id} ({e}); continuing without it.")
                cbo_context = ""
            classification = classify(title, status, summary_text, cbo_context, bill_id=bill_id)
            ...
            impact_score, market_relevance = compute_impact_score_and_relevance(
                classification["impact_breakdown"], has_summary)
            classification["has_summary"] = has_summary
            classification["market_relevance"] = market_relevance
            classified += 1
        ...
        signals.append({
            ...
            "impact_breakdown": classification["impact_breakdown"],
    ...
    summary_core = {
        ...
        # Counts market_relevance == "HIGH", not just impact_score >= 70 -
        # a bill only counts here if it's ALSO backed by a real published
        # summary. A WATCH bill that happens to score 70+ on thin/no data
        # doesn't count until it has real information behind it. See
        # compute_impact_score_and_relevance() for the full logic.
        "high_impact": sum(1 for s in signals if s["market_relevance"] == "HIGH"),
        "watch_list": sum(1 for s in signals if s["market_relevance"] == "WATCH"),
    ...
    week_over_week_deltas = {
        ...
        # .get() rather than direct indexing - a week_ago entry recorded
        # before this field existed won't have "watch_list" at all.
        "watch_list": (summary_core["watch_list"] - week_ago["watch_list"])
                      if week_ago and "watch_list" in week_ago else None,
    }
